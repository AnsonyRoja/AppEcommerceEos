...
class _direccionHabitacionState extends State<DireccionHabitacion> {
  ...
  var campo = {'cities_id': null, 'address': '', 'urb': '', 'zip_code': '', 'sector': '', 'nro_home': '', 'reference_point': ''};
  bool loading = false;
                                  ...
                                  try {
                                    ...
                                    return Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        _campoTexto("urb", "Urbanización/Barrio/Empresa", 'todo', true, row['urb']),
                                        Row(
                                          children: <Widget>[
                                            Expanded(
                                                child: Padding(
                                                    padding: EdgeInsets.only(right: 5),
                                                    child: _campoTexto("zip_code", "Codigo postal", 'numero', true, row['zip_code']))),
                                            Expanded(child: _campoTexto("sector", "Sector/Avenida/Calle", 'todo', true, row['sector'])),
                                          ],
                                        ),
                                        Row(
                                          children: <Widget>[
                                            Expanded(
                                                child: Padding(
                                              padding: EdgeInsets.only(right: 5),
                                              child: _campoTexto("nro_home", "Nro de casa/local", 'todo', true, row['nro_home']),
                                            )),
                                            Expanded(child: _campoTexto("reference_point", "Punto de referencia", 'todo', true, row['reference_point'])),
                                          ],
                                        ),
                                        Padding(
                                        ...
                                        Text(
                                            "La dirección de habitación no aplica para el delivery, si desea registrar una dirección para delivery ingrese en el menú principal -> direcciones de entrega."),
                                        _botonEnvio(row['id'])
                                      ],
                                    );
                                  } catch (e) {
    ...
    return Padding(
        ...
        child: Center(
            child: ElevatedButton(
                ...
                child: _btnDireccionesCargando == true
  ...
  items: data.map((item) {
    return DropdownMenuItem<String>(
      ...
      value: item['id'].toString(), // Asegúrate de convertir 'id' a String si es necesario
    );
  }).toList(),
  hint: Text("Parroquia"),
  onChanged: (newVal) {
    setState(() {
      campo['cities_id'] = newVal; // 'newVal' ya es de tipo String
      _cities_id = newVal;
  ...
  Widget _selectStatesId(List data) {
    ...
    return DropdownButton<String>(
      items: data?.map((item) {
            if (item['id'] == _states_id) {
              //para evitar el error si desactiamos un estado y el usuario consulta y tiene el estado viejo
              _buenoStates = true;
            }
            return DropdownMenuItem<String>(
              child: Text(item['name']),
