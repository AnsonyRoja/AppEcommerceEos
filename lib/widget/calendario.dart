...
class _calendarioState extends State<Calendario> {
 ...
 DateTime selectedDate = DateTime.now().subtract(Duration(days: 3600));
  ...
  Future<Null> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      locale: Locale('es'),
        ...
        lastDate: DateTime.now().subtract(Duration(days: 3590)));
  ...
  Widget build(BuildContext context) {
   ...
   _controller = TextEditingController(text: primeraVez ? '' : fecha);
    return _campoFecha();
  }
  _campoFecha(){
    Padding(
