...
peticionGetZlib(url) async {
  ...
  try {
    ...
    print('Esto es la respuesta ${response.bodyBytes}');
    try {
     
      data = utf8.decode(response.bodyBytes);

      print("bueno esto es la data $data");
    }catch(e){
      print(e);
      print("malo");
      data=response.body;
...
getData(key) async{
  print('Entre aqui en getData ');
  SharedPreferences prefs = await SharedPreferences.getInstance();
  print('esto es el prefs $prefs y esta es la key $key');
...
peticionPost(url, Map<String, String> body) async {
  print('Entre aqui en la funcion peticionPost ');
   print(body);
  try {
    var uri = Uri.parse(url);
...
getEvento() async {
  ...
  print('Este es el evento en la function getEvento() ');
  return evento;
}
getTitulo() async {
...
final formatDolarSinSimbolo = NumberFormat.simpleCurrency(locale: 'en_US',name:'',decimalDigits: 2);
...
final formatBolivarSinSimbolo = NumberFormat.simpleCurrency(locale: 'es_ES',name: '',decimalDigits: 2);